...
class MetaContactForm(forms.Form):
    ...
    verify = forms.CharField(label = '', max_length=100, widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'verify'}))
    metadata = forms.CharField(label = '', widget=forms.TextInput(attrs={'readonly':'readonly', 'id': 'metadata', 'hidden': 'true'}))
    return_url = forms.CharField(label = '', max_length=100, widget=forms.TextInput(attrs={'readonly':'readonly', 'id': 'return_url' , 'hidden': 'true'}))
